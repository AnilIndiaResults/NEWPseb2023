...
namespace PSEBONLINE.AbstractLayer
{
    public class ReportDB
    {
        ...
        public DataSet DistWiseSchoolReport(string Dist)
        {
            DataSet result = new DataSet();
            SqlDataAdapter ad = new SqlDataAdapter();
            try
            {
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings[CommonCon].ToString()))
                {
                    SqlCommand cmd = new SqlCommand("DistWiseSchoolReportSP", con);
                    cmd.CommandType = CommandType.StoredProcedure;
        ...
        public DataSet ClassSubjectWiseRegistrationReportSP(int type, string RP, string DIST, int cls)
        {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings[CommonCon].ToString()))
                {
                    SqlCommand cmd = new SqlCommand("ClassSubjectWiseRegistrationReportSP", con);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.CommandTimeout = 300;
                    cmd.Parameters.AddWithValue("@type", type);
                    cmd.Parameters.AddWithValue("@RP", RP);
