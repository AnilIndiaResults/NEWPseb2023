    ...
    public class EAffiliationDB
    {
                       ...
                       if (ds.Tables[0].Rows.Count > 0)
                        {
                            ...
                            eAffiliationDashBoardViewModel.appliedForClass = dr["appliedForClass"].ToString();
                            eAffiliationDashBoardViewModel.eaffCategory = dr["eaffCategory"].ToString();
                            eAffiliationDashBoardViewModel.EAffSession = dr["EAffSession"].ToString();
        ...
        public DataSet GetEAffiliationPayment(string appno)
        {
            ...
            try
            {
                using (SqlConnection con = new SqlConnection(ConfigurationManager.ConnectionStrings[CommonCon].ToString()))
                {
                    //SqlCommand cmd = new SqlCommand("GetEAffiliationPaymentSP", con); GetEAffiliationPaymentSP_temp
                    SqlCommand cmd = new SqlCommand("GetEAffiliationPaymentSP_temp", con);
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@appno", appno);
                    ad.SelectCommand = cmd;
                    ad.Fill(result);
