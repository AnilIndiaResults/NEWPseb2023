...
namespace PSEBONLINE.Controllers
{
    public class LoginController : Controller
    {
        ...
        public ActionResult Index(LoginModel lm)
        {
            try
            {
                ...
                if (loginSession != null)
                {
                    ...
                    Session["firstLastSession"] = years[0];
                    Session["SecondLastSession"] = years[1];
                    ...
                    if (loginSession.LoginStatus == 1)
                    {
                        ...
                        if (ds.Tables.Count > 0)
                        {
                            ...
                            if (dt.Rows[0]["Active"].ToString() != "")
                            {
                                ...
                                //var controller = DependencyResolver.Current.GetService<TestcallapiController>();
                                ...
                                //cmd.CommandText = "getChallan"; //InsertOnlinePaymentMISSP // InsertOnlinePaymentMISSPNEW
                                //cmd.Parameters.AddWithValue("@schlregid", dt.Rows[0]["schl"].ToString());
                                //dataAdapter.SelectCommand = cmd;
                                //dataAdapter.Fill(results); bool paymentstatus = false;
                                //foreach (DataRow dr in results.Tables[0].Rows)
                                //{
                                //    int amt = -1;
                                //    Int32.TryParse(dr["totfee"].ToString(), out amt);
                                //    paymentstatus = controller.callBackAPI(dr["challanid"].ToString(), amt);
                                //}
        ...
        public string getlastSessions(string input)
        {


            // Split the input string by the hyphen ("-") to get the two years
            ...
            if (years.Length == 2)
            {
                ...
                if (int.TryParse(years[0], out int startYear) && int.TryParse(years[1], out int endYear))
                {
                    ...
                    string firstlastYear = $"{startYear}-{endYear}";
                    string secondLastYear = $"{startYear - 1}-{endYear - 1}";
                    string newYearRange = firstlastYear + "," + secondLastYear;
                    // Print the result
                    return newYearRange;
                }
                else
                {
                    return ("Invalid year format in the input string.");
                }
